...
def pipeline() -> None:
    ...
    df = pd.read_csv('/opt/airflow/data/train/homework.csv')  # C:\Users\HP Z2\airflow_hw\dags\data\train\homework.csv
    ...
    best_pipe.fit(X, y)
    model_filename = f'{path}/models/cars_pipe_{datetime.now().strftime("%Y%m%d%H%M")}.pkl'
